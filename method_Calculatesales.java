...
public class method_Calculatesales {
...
private static  boolean readTeigiFile(String teigiName,File fileName,String seikiFormat,
 HashMap<String, String> mapBraCom,HashMap<String, Long> rcdBraComMap) {
...
private static  boolean amountCheck(String teigiName, String fileName,long amount,
		HashMap<String, Long> amountRcdMap, String code) {
	
	// 定義ファイル、コードエラー処理
	if (!amountRcdMap.containsKey(code)) {
		System.out.println(fileName + "の"+ teigiName + "コードが不正です");
		return false;
	}
...
//「メソッド」商品別集計ファイルの作成
private static boolean writeRcd(File fileName, List<Map.Entry<String, Long>> sortTeigi, 
		HashMap<String, String> teigiMap){
		BufferedWriter bw = null;
try {
	bw = new BufferedWriter(new FileWriter(fileName));
	fileName.createNewFile();
	
	for (Entry<String, Long> e : sortTeigi) {
		bw.write(e.getKey() + "," + teigiMap.get(e.getKey()) +","+ e.getValue());
		bw.newLine();
	}
} catch (IOException e) {
	System.out.println("予期せぬエラーが発生しました");
...
//「メソッド」ソート降順
private static List<Entry<String,Long>> sortRcd(Map<String, Long> teigiRcdMap) {
	List<Map.Entry<String, Long>> sortTeigi = new ArrayList<Map.Entry<String,Long>>(teigiRcdMap.entrySet());
	Collections.sort(sortTeigi, new Comparator<Map.Entry<String,Long>>() {
		@Override
...
//「メソッド」売上抽出
static void rcdChusyutu(ArrayList<String> rcdFile, File[] fileName,String kensakuFormat,
		ArrayList<Integer> rcdNomber){
	
	// 売上ファイル抽出
	for (File value : fileName) { 
		File inputFile = value;
		if (inputFile.getName().matches("kensakuFormat")) { // 数字8桁、rcdファイル検索
			if(inputFile.isFile()){
